...
export const ProfileCard = ({ location }) => {
    ...
    const handleApproveFriend = async (eBtn) => {
        let data = await ContactAPI.approveRequestContact(
            currentUser.token,
            id
        );
    ...
    const handleRemoveContact = (eBtn) => {
        swal({
            ...
            if (btnAction) {
                const data = await ContactAPI.removeContact(
                    currentUser.token,
                    id
                ); //id of userr contact
                console.log(data);
                if (data.data) {
                    ...
                    dispatch(
                        alertt_success(
                            "You have just successfully deleted this friend"
                        )
                    );
                }
            }
    ...
    const handleRemoveReqContactSent = async (eBtn) => {
        const data = await ContactAPI.removeReqContactSent(
            currentUser.token,
            id
        ); //id of userr contact
        if (data.data) {
