  ...
  return (
    <div>
      <div className="follower" style={{ cursor: "pointer" }}>
        <div
          onClick={() => sendToProfile(contactUser._id)}
          id={contactUser._id}
          name={contactUser._id}
        >
          <img
            src={`${process.env.REACT_APP_AVATAR_IMAGE_FOLDER}${contactUser.avatar}`}
            alt=""
            className="followerImg"
          />

          <div className="name">
            <span>{contactUser.firstName}</span>
            <span>{contactUser.email}</span>
          </div>
        </div>
        <button
          className="button followButton"
          onClick={() => handleAddContact(contactUser._id)}
        >
          Add friend
        </button>
      </div>
