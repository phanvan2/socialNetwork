...
UserSchema.statics = {
  ...
  findUserById(id) {
    return this.find(
      { _id: id },
      "_id email lastName firstName avatar isActive livesin country workAt gender relationship createAt"
    ).exec();
  },
  getUserAll() {
    return this.find(
      {},
      "_id email lastName firstName avatar livesin country workAt gender relationship createAt"
    ).exec();
  },
  ...
  findAllForAddContact(deprecatedUserIds, keyword) {
    return this.find(
      ...
      { _id: 1, email: 1, lastName: 1, firstName: 1, avatar: 1 }
    ).exec();
  },
  ...
  getListFriends(friendIds) {
    return this.find(
      {
        $and: [{ _id: { $in: friendIds } }, { isActive: true }],
      },
      { _id: 1, email: 1, lastName: 1, firstName: 1, avatar: 1 }
    ).exec();
  },
