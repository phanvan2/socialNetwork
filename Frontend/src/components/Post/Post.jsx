...
export const Post = ({ data, location=null }) => {
  ...
  const handleDeletePost = () => {
    ...
    //       if(result.data){
    //           dispatch(alertt_success("You have just successfully deleted this post"));
    //           navigate("/home")
  ...
  return (
    <div className='Post'  onClick={()=> {if(user.data._id !== data.userId || location !== "profile") sendToPost(data._id)  } } >
        <div className='card-post-header'>
          <div className='post-header' >
            <div className='detail-post-header'  onClick={()=> sendToPost(data._id) }>
              ...
              <span className='desc-post'>{data.lastName}</span>
              <span className='createTime-post'>{convertTimestampToHumanTime(`${data.creatAt}`)}</span>
              </div>

            </div>
            <div>
            {user.data._id === data.userId && location == "profile" ? (
                  <>
              ...
              >
                  ...
                  <Typography className='btn-edipost' sx={{ p: 2 }} onClick={()=> {setOpenDialog(true); handleClose()}}>Edit Post.</Typography>
                  <Typography className='btn-edipost' sx={{ p: 2 }} onClick={()=> {handleDeletePost()}}>Delete Post.</Typography>
      ...
      <span style={{"fontSize": "14px", "color": "#575454"}}>{parse(data.desc)}</span>
        ...
        >
            ...
            <form className='infoForm' action="">
                ...
                <div style={{"marginTop": "105px"}}>
                    Profile Image
                    <input type="file" name='profilePicture' onChange={(e) => { setFile_(e.target.files[0])}}/>
                   
                </div>
                <button className='button infoButton' onClick={handleUpdatePost}>Update</button>
            </form>
        </Modal>
