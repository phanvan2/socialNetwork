  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      result.forEach((value, index) => {
        if (value.city) {
          if (citydata.indexOf(value.city) < 0) citydata.push(value.city);
        }
        if (value.workat) {
          if (workAt.indexOf(value.workat) < 0) workAt.push(value.workat);
        }
        if (value.gender) {
          if (gender.indexOf(value.gender) < 0) gender.push(value.gender);
        }
          ...
          callbacks: [
            {
              onEpochEnd: (epoch, logs) => {
                if (logs.acc == 1) {
                  ...
                  console.log("Đạt ngưỡng validation loss, dừng đào tạo.");
