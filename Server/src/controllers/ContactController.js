  ...
  if (req.query.user_token) {
    try {
      ...
      if (users.length > 0) {
        return res
          .status(200)
          .send({ data: users, message: transSuccess.findContact });
      } else {
...
let addNew = async (req, res) => {
  if (!_.isEmpty(req.body)) {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ data: newContact, message: transSuccess.sendReqContact });
    } catch (error) {
      return res.status(500).send(error);
    }
  }
  {
    return res
      .status(200)
      .send({ data: false, message: transError.sendReqContact });
  }
};
...
let removeContact = async (req, res) => {
  if (!_.isEmpty(req.body)) {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ data: removeContact, message: transSuccess.sendReqContact });
    } catch (error) {
...
let countFriend = async (req, res) => {
  try {
    if (req.query.idUser) {
      let number = await Contact.countFriend(req.query.idUser);
      return res.status(200).send({ numberFriend: number });
    } else {
      res.send(false);
    }
  } catch (error) {
