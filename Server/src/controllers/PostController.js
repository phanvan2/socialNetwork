...
import { Post, train, Contact } from "../services";
            ...
            if (_.isEmpty(req)) {
                ...
                try {
                    ...
                    if (result) {
                        res.status(200).send({
                            data: true,
                            message: transSuccess.addNewPost,
                        });
                    } else {
                        res.send({
                            data: false,
                            message: transError.addNewPost,
                        });
                    }
                } catch (error) {
                    console.log(error);
...
let getPostsByListfriend = async (req, res) => {
    if (req.query.user_token) {
        try {
            let req_user = jwt.verify(
                req.query.user_token,
            ...
            let currentUserId = req_user._id;
            let posts = await Post.getPostsByListfriend(currentUserId);
            res.send(posts);
            // if(posts.length > 0){
            //     return res.status(200).send(posts);
...
let trainModelCollaborative = async (req, res) => {
    let result = await train.trainModelCollaborative();
    res.send(result);
};
